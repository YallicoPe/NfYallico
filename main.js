...
global.authFile = `YallicoSession`
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['YallicoSession', 'Edge', '2.0.0'] : methodCodeQR ? ['YallicoSession', 'Edge', '2.0.0'] : ['Chrome (Linux)', '', ''],
auth: {
creds: state.creds,
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "YallicoSession"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
if (reason == 405) {
await fs.unlinkSync("./YallicoSession/" + "creds.json")
...
let prekey = []
let directorio = readdirSync("./YallicoSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./YallicoSession/${files}`)
})
} 

function purgeSessionSB() {
try {
const listaDirectorios = readdirSync('./YallicoSession/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./YallicoSession/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./YallicoMd/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./YallicoSession/${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./YallicoSession/', './YallicoMd/']
