...
const handler = async (m, { conn, text }) => {
  try {
          ...
          if (messager.includes('Updating')) messager = '𝚈𝚊𝚕𝚕𝚒𝚌𝚘 𝙱𝚘𝚝 🇦🇱\n\n' + stdout.toString()
 ...
 try {    
      ...
      if (status.length > 0) {
        const conflictedFiles = status
          ...
          .map(line => {
            if (line.includes('.npm/') || line.includes('.cache/') || line.includes('tmp/') || line.includes('YallicoSession/') || line.includes('npm-debug.log')) {
              return null;
            }
            return '*◉ ' + line.slice(3) + '*';
          })
